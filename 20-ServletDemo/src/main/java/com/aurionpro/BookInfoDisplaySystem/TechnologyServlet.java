...
@WebServlet(urlPatterns = "/technology", initParams = { @WebInitParam(name = "totalTechnologyVistors", value = "0") })
public class TechnologyServlet extends HttpServlet {
	private int totalTechnologyVistors;

	@Override
	public void init() throws ServletException {
		ServletConfig config = getServletConfig();
		totalTechnologyVistors = Integer.parseInt(config.getInitParameter("totalTechnologyVistors"));
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		out.println("<h1>BookVerse - Technology Category Info</h1>");
		out.println("<table>");
		...
		out.println("<tr><td>Technology Books Visited</td><td>" + totalTechnologyVistors + "</td></tr>");
		out.println("<tr><td>Total Visitors</td><td>" + totalVisits + "</td></tr>");
		out.println("<tr><td>App Version</td><td>1.2.3</td></tr>");
		out.println("</table>");

		out.println("<br><br>");
		out.println("<form action='" + req.getContextPath() + "/admin/technology' method='get' style='display:inline;'>");
