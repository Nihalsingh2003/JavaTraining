...
public class AdminDeleteProductPage extends HttpServlet {
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		out.println("<title>Delete Products</title>");
		out.println("<style>");
		...
		out.println(
				"select, button { width: 100%; padding: 12px 10px; margin-top: 20px; border-radius: 5px; border: 1px solid #ccc; font-size: 16px; }");
		...
		out.println("<h1>Select Products to Delete</h1>");

		out.println("<form method='Post' action='deleteProductHandler'>");

		out.println("<label for='categorySelector'><strong>Select Category:</strong></label>");
		out.println("<select id='categorySelector' name='selectedCategoryName' onchange='showTab(this.selectedIndex)'>");
		for (int i = 0; i < categoryNames.size(); i++) {
			out.println("<option value='" + categoryNames.get(i) + "'>" + categoryNames.get(i) + "</option>");
		}
		out.println("</select>");
		...
		for (int i = 0; i < productList.size(); i++) {
			...
			if (categoryProducts.isEmpty()) {
				...
				for (String product : categoryProducts) {
					out.println("<label><input type='checkbox' name='productsToDelete' value='" + product + "'> "
							+ product + "</label>");
				}
			}
			out.println("</div>");
		...
		out.println(
			"<button type='submit' style='background-color:#dc3545; color:white; padding:6px 12px; font-size:14px; border:none; border-radius:4px; cursor:pointer; width:auto;'>Delete Selected Products</button>");

		out.println("</form>");

		out.println("<div style='text-align:center; margin-top: 20px;'>");
		out.println("<a href='adminHome' style='text-decoration:none;'>");
		out.println("<button style='background-color:#007bff; color:white; padding:6px 12px; font-size:14px; border:none; border-radius:4px; cursor:pointer; width:auto;'>Go Back to Dashboard</button>");
		out.println("</a>");
		out.println("</div>");
